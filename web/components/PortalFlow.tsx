...
const COPY: Record<State, { status: string; title: string; body: string }> = {
  ...
  denied: {
    ...
    body: "That wallet holds no membership here and no organization-scoped role. Ask an organizer to onboard you.",
  },
};
  ...
  return (
    <div className="w-full max-w-[400px]">
      <section className="rounded-sharp border border-rule bg-paper-raise">
        ...
        {/* Signal band — instrumentation, sized like a readout rather than art */}
        ...
        <div className="relative overflow-hidden">
          <AnimatePresence mode="wait" initial={false}>
            ...
            >
              ...
              <div className="mt-6">
                {state === "idle" ? (
                  <Button variant="solid" className="w-full" onClick={() => go("signing")}>
                    Connect wallet
                  </Button>
                ) : null}
                {state === "signing" ? (
      ...
      {/* Demo affordance, deliberately subordinate to the portal itself */}
        ...
        <div className="mt-3 flex flex-wrap gap-1.5">
          {ORDER.map((s) => (
            <button
              ...
              className={`inline-flex min-h-11 items-center rounded-sharp border px-3 font-mono text-[0.6875rem] uppercase tracking-[0.1em] transition-colors ${
                state === s
